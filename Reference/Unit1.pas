...
uses
  ...
  irbis64_client, StdCtrls, Buttons, ExtCtrls;
...
begin
GetMem(rp,32000);
StrCopy(rp,Pchar(Memo1.Lines.Text));
...
Caption:=IntToStr(ri);
TL.Text:=String(rp);
Memo3.Lines.Text:=String(rp);
Memo1.Text:=String(IC_delim_reset(Pchar(TL.Strings[0])));
Memo2.Text:=String(IC_delim_reset(Pchar(TL.Strings[1])));
FreeMem(rp);
TL.Free;
end;
...
Memo3.Lines.Text:=String(rp);
//Memo2.Lines.Text:=Utf8ToWideString(String(rp1));
FreeMem(rp);
FreeMem(rp1);
TL.Free;
end;
