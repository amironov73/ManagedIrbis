...
namespace UnitTests.AM.Runtime
{
    ...
    public class HandmadeSerializerTest
    {
        ...
        [TestMethod]
        public void HandmadeSerializer_Roundtrip()
        {
            ...
            {
                MyFirstClass first2
                    = (MyFirstClass) serializer.Deserialize(reader);
                Assert.AreEqual(first1.FirstField, first2.FirstField);

                MySecondClass second2
                    = (MySecondClass) serializer.Deserialize(reader);
                Assert.AreEqual(second1.SecondField, second2.SecondField);

                MyThirdClass third2
                    = (MyThirdClass) serializer.Deserialize(reader);
        ...
        [TestMethod]
        public void HandmadeSerializer_Array()
        {
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                IHandmadeSerializable[] tempArray
                    = serializer.DeserializeArray(reader);
                MyFirstClass[] targetArray
                    = tempArray.Cast<MyFirstClass>().ToArray();
