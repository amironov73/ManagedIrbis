...
namespace UnitTests.ManagedIrbis
{
    ...
    public class FastFieldsTest
    {
        ...
        [TestMethod]
        public void FastFields_GetFields_1()
        {
            RecordField[] fields = new RecordField[0];
            FastFields fast = new FastFields(fields);

            for (int tag = 0; tag < 2000; tag++)
            {
                RecordField[] found = fast.GetFields(tag);
                Assert.IsNull(found);
            }
        }

        [TestMethod]
        public void FastFields_GetFields_2()
        {
            MarcRecord record = GetRecord();
            FastFields fast = new FastFields(record.Fields);
            ...
            for (int tag = 0; tag < 2000; tag++)
            {
                ...
                if (record.HaveField(tag))
                {
                    ...
                    Assert.AreEqual(record.Fields.GetFieldCount(tag), found.Length);
                }
                else
                {
                    Assert.IsNull(found);
        ...
        [TestMethod]
        public void FastFields_GetField_1()
        {
            RecordField[] fields = new RecordField[0];
            FastFields fast = new FastFields(fields);
            ...
            for (int tag = 0; tag < 2000; tag++)
            {
                ...
                {
                    RecordField found = fast.GetField(tag, occ);
                    Assert.IsNull(found);
                }
            }
        }
        ...
        [TestMethod]
        public void FastFields_GetField_2()
        {
            MarcRecord record = GetRecord();
            FastFields fast = new FastFields(record.Fields);
            ...
            for (int tag = 0; tag < 2000; tag++)
            {
                ...
                if (record.HaveField(tag))
                {
                    int max = record.Fields.GetFieldCount(tag);
                    for (int occ = 0; occ < max; occ++)
                    {
                        found = fast.GetField(tag, occ);
                        Assert.AreSame(record.Fields.GetField(tag, occ), found);
                    }
                    for (int occ = max; occ < 10; occ++)
                    {
                        found = fast.GetField(tag, occ);
                        Assert.IsNull(found);
                    }
