...
namespace ManagedIrbis.Infrastructure.Commands
{
    ...
    //
    // Имя пользователя, выполнившего интерактивный вход в систему:
    // System.Environment.UserName
    // (без домена и прочего)
    //
    // Текущий пользователь, от имени которого выполняется процесс
    // (это необязательно может быть пользователь, выполнивший
    // вход в систему):
    // WindowsIdentity.GetCurrent().Name 
    // в формате domain\username
    // Для анонимного пользователя выдается null.
    // Для системы выдается ComputerName\LocalSystem или .\LocalSystem
    // или просто LocalSystem. SID: S-1-5-18
    // Для локального сервиса NT AUTHORITY\LocalService. SID: S-1-5-19
    // Для сетевого сервиса NT AUTHORITY\NetworkService.  SID: S-1-5-20
    //
